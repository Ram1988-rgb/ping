 ...
 *   contributors: Guy Bedford <guybedford@gmail.com>, Ionut-Cristian Florescu <ionut.florescu@gmail.com>, James Halliday <mail@substack.net>, Jan SchÃ¤r <jscissr@gmail.com>, Johannes Ewald <johannes.ewald@peerigon.com>, Jonathan Prins <jon@blip.tv>, Matt Esch <matt@mattesch.info>
      ...
      var aTemplate = function () {
        ...
        _createClass(aTemplate, [{
          ...
          value: function getDataFromObj(s, o) {
            s = s.replace(/\[([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+)\]/g, '.$1'); // convert indexes to properties
          ...
          value: function resolveBlock(html, item, i) {
            var that = this;
            var touchs = html.match(/<!-- BEGIN ([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):touch#([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+) -->/g);
            var touchnots = html.match(/<!-- BEGIN ([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):touchnot#([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+) -->/g);
            var exists = html.match(/<!-- BEGIN ([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):exist -->/g);
            var empties = html.match(/<!-- BEGIN ([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):empty -->/g);
            /* ã‚¿ãƒƒãƒãƒ–ãƒ­ãƒƒã‚¯è§£æ±º*/
            if (touchs) {
              for (var k = 0, n = touchs.length; k < n; k += 1) {
                var start = touchs[k];
                start = start.replace(/([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):touch#([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+)/, '($1):touch#($2)');
            ...
            /* ã‚¿ãƒƒãƒãƒŽãƒƒãƒˆãƒ–ãƒ­ãƒƒã‚¯è§£æ±º*/
            if (touchnots) {
              for (var _k = 0, _n = touchnots.length; _k < _n; _k += 1) {
                var _start = touchnots[_k];
                _start = _start.replace(/([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):touchnot#([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+)/, '($1):touchnot#($2)');
            ...
            /* existãƒ–ãƒ­ãƒƒã‚¯ã‚’è§£æ±º*/
            if (exists) {
              for (var _k2 = 0, _n2 = exists.length; _k2 < _n2; _k2 += 1) {
                var _start2 = exists[_k2];
                _start2 = _start2.replace(/([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):exist/, '($1):exist');
            ...
            /* emptyãƒ–ãƒ­ãƒƒã‚¯ã‚’è§£æ±º*/
            if (empties) {
              for (var _k3 = 0, _n3 = empties.length; _k3 < _n3; _k3 += 1) {
                var _start3 = empties[_k3];
                _start3 = _start3.replace(/([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+):empty/, '($1):empty');
            ...
            /* å¤‰æ•°è§£æ±º*/
            html = html.replace(/{([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+)}(\[([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+)\])*/g, function (n, key3, key4, converter) {
              var data = void 0;
              if ('' + key3 === 'i') {
                data = i;
          ...
          //   // var width = /< !--BEGIN([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ - ç†™] +):with --> (([\n\r\t] |.) *?) < !--END([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ - ç†™] +): with --> /g;
          ...
          value: function resolveLoop(html) {
            var loop = /<!-- BEGIN ([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+?):loop -->(([\n\r\t]|.)*?)<!-- END ([\w\-\.ã-ã‚“ã‚¡-ãƒ¶äºœ-ç†™]+?):loop -->/g;
            var that = this;
            /* ãƒ«ãƒ¼ãƒ—æ–‡è§£æ±º*/
          ...
          value: function getHtml(query, row) {
            ...
            /* çµ¶å¯¾ãƒ‘ã‚¹ã§æŒ‡å®šã•ã‚ŒãŸå¤‰æ•°ã‚’è§£æ±º*/
