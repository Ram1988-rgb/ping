...
class Secureapi extends REST_Controller {
  ...
  public function investment_summary_get($investment_id){
    ...
    $all_data = array(
      ...
      'next_payment_amount'=>$line->amount,
      'all_payment_date'=>$all_payment_date,
      'mybalence'=>$this->investment_model->get_total_investment($line->plan_id,$this->customer_id)->amount,
    ...
    if($all_list){
      foreach($all_list as $line){
        $all_data[] = array(
          ...
          'installment_amount'=>$line->install_amount,
          'start_date'=>$line->start_date,
          'end_date'=>$line->end_date,
          'interest_rate'=>$line->interest_rate,
          'payment_type_id'=>$line->payment_type_id,
  ...
  public function loan_summary_get($loan_id){
    ...
    if($line){ 
      ...
      $all_data = array(
        ...
        'payment_duration'=> ($line->payment_deuration_data != "null")?JSON_DECODE($line->payment_deuration_data)->label:'',
        'next_payment_date'=> isset($next_payment_date)?$next_payment_date->payment_date:'',
        'next_payment_amount'=>$line->install_amount,
        'all_payment_date'=>$all_payment_date
      );
        
      $response_data = array('status'=> 200, 'message' => '', "data"=>$all_data, 'total_loan'=>$this->loan_model->get_total_loan($this->customer_id)->amount);
      $this->response($response_data, REST_Controller::HTTP_OK);  
    }else{
  ...
  public function createinvestment_post(){
    ...
    $paymentDuration = getRecord($_POST['payment_duration'], TBL_PAYMENTDURATION);
    ...
    if($addId=$this->investment_model->create_plan($this->input->post())){
  ...
  public function createloan_post(){
    ...
    if($loan_id = $this->loan_model->addLoan($this->input->post())){
  ...
  public function fund_recent_activity_get(){
    ...
    $response_data = array('status'=> 200, 'message' => '', "data"=>array('wallet_amount'=>"".getCurrentBalence($this->customer_id),
  ...
  public function verify_nif_post(){
    ...
    if($this->input->post()){
     ...
     if(isset($_FILES['nif_image']['name'])){
       ...
       $addNif = $this->home_model->addNIF($this->input->post(),$this->customer_id);
       if($addNif){
  ...
  public function add_card_post(){
    $_POST['user_id'] = $this->customer_id;
    $card =  $this->card_model->add_card($this->input->post());
    if($card){
  ...
  public function cashout_post(){    
    if($this->input->post()){
      ...
      if(isset($_POST['bank_id'])){
        $_POST['bank_detail'] = $this->bank_model->bank_detail($_POST['bank_id']);
      }
      ...
      $_POST['agent_id']=$this->agent_model->checkAgentCode($_POST);
      if(!$_POST['agent_id']){
      ...
      $data =  $this->cashout_model->addcashout($this->input->post());
  ...
  public function refer_app_post(){
    if($this->input->post()){
      $_POST['user_id'] = $this->customer_id;
      $add = $this->referapp_model->add($this->input->post());
  ...
  public function edit_bank_post($id){
    $_POST['user_id'] = $this->customer_id;
    if($this->bank_model->editBank($this->input->post(),$id)){
  ...
  public function add_friend_post(){
    $_POST['user_id'] = $this->customer_id;
    if($this->user_model->checkFriends($_POST)){
      $response_data = array('status'=> 200, 'message' => 'Your request has been already sent', "data"=>[]);
      $this->response($response_data, REST_Controller::HTTP_OK);
      return true; 
    }
    if($this->user_model->addFriend($this->input->post())){
      $response_data = array('status'=> 200, 'message' => 'Your request has been sent successfully', "data"=>[]);
      $this->response($response_data, REST_Controller::HTTP_OK);
      return true; 
    }
  ...
  public function cash_data_get(){
    ...
    $response_data = array('status'=> 200, 'message' => '', "data"=>array(
      'plan'=>$data,
      'mybalance'=>array('amount'=>$this->cash_model->get_cash_saving($this->customer_id))
    ));
    $this->response($response_data, REST_Controller::HTTP_OK);
    return true; 
  }
  ...
  public function createcash_post(){
    $_POST['user_id'] = $this->customer_id;
    $_POST['plan_id'] = 1;
    if($insId = $this->cash_model->save_data($_POST)){
  ...
  public function tofa_post(){
    ...
    if($this->home_model->update_tofa($_POST)){     
