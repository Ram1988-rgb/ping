...
public function changepassword(){
    if($_POST){
       ...
       $res =  $this->login_model->changePassword($this->security->xss_clean($this->input->post()));
       if($res){
...
public function checkPassword(){
    $_POST['adminId'] = $this->session->userdata('ADMINID');
    $result = $this->login_model->checkPassword($this->security->xss_clean($this->input->post()));
    if(!$result){
        $response = array(
            'valid' => false,
