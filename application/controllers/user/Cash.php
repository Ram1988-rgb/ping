...
{ 
  ...
  if($this->input->post()){    
    ...
    if($insId = $this->cash_model->save_data($this->security->xss_clean($this->input->post()))){
...
public function get_plan_duration(){  
  try{
    $plan_id = $this->security->xss_clean($_POST['plan_id']);
    $duration_id = $this->security->xss_clean($_POST['duration_id']);
    $data = [];
...
function cash_payment(){
  $cash_payment_date_id =  $this->security->xss_clean($this->input->post('cashPayment_date_id'));
...
public function check_plan_duration(){  
  try{
    ...
    $payment_type = isset($_POST['payment_type'])?$this->security->xss_clean($_POST['payment_type']):'';
    $duration_id = $this->security->xss_clean($_POST['payment_duration']);
