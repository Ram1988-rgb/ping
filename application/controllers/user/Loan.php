...
{ 
    ...
    if($this->input->post()){
         ...
         $addLoan = $this->loan_model->addLoan($this->security->xss_clean($this->input->post()));
        // die;
        if($addLoan){          
...
public function myloan(){
    $_POST['user_id'] = $this->session->userdata('CUSTOMERID');
    $this->data['all_record'] = $this->loan_model->get_all_loan_record($this->security->xss_clean($_POST['user_id']));
...
public function save_data(){
	$this->data['type_id'] = $this->security->xss_clean($this->input->post('type'));
	$this->data['loan_duration'] = $this->plan_model->get_plan_type_duration($this->security->xss_clean($this->input->post('type')));
    $this->data['payment_duration'] = $this->plan_model->get_plan_payment_duration($this->security->xss_clean($this->input->post('type')));
...
public function record_save(){
	...
	if($this->input->post()){
      $addFund = $this->loan_model->addLoan($this->security->xss_clean($this->input->post()));
      if($addFund){
...
public function getloanform(){
    $plan_id = $this->security->xss_clean($this->input->post('plan_id'));
    $this->data['plan_id'] = $this->security->xss_clean($this->input->post('plan_id'));
    $this->data['loan_duration'] = $this->plan_model->get_plan_type_duration($this->security->xss_clean($this->input->post('plan_id')));
    $this->data['payment_duration'] = $this->plan_model->get_plan_payment_duration($this->security->xss_clean($this->input->post('plan_id')));
    //print_r($this->data); die();
...
public function loan_payment_data(){
    $loan_payment_date_id = $this->security->xss_clean($_POST['loan_date_id']);
