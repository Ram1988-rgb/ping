	...
	$this->header_data['headTitle'] = "Nuvest : Customer Add Bank";    
     if($this->input->post()){
		 $_POST['user_id'] = $this->session->userdata('CUSTOMERID');
		  $addBank = $this->bank_model->addBank($this->security->xss_clean($this->input->post()));
		  if($addBank){
			  sendNotificationProfileBank($this->security->xss_clean($_POST), 'WEB');
	...
	$this->header_data['headTitle'] = "Nuvest : Customer Verify NIF";     
     if($this->input->post()){
		...
		if(isset($_FILES['nif_image']['name'])){
		  ...
		  $_POST['user_id'] = $this->session->userdata('CUSTOMERID');
		  $addNif = $this->home_model->addNIF($this->security->xss_clean($this->input->post()),$this->session->userdata('CUSTOMERID'));
		  if($addNif){
			$data = array(
				'status'=>3,
				'doc_type'=>'Nif',
...
function edit_profile(){
	...
	if($this->input->post()){
		...
		$edit = $this->user_model->editUser($this->security->xss_clean($this->input->post()),$this->security->xss_clean($_POST['user_id']));
		if($edit){
			$newdata = array(
				'CUSTOMERNAME' => $this->security->xss_clean($_POST['name']),                        
				'CUSTOMERPHONE' => $this->security->xss_clean($_POST['phone']), 
				'CUSTOMEREMAIL' => $this->security->xss_clean($_POST['email']),                       
				'CUSTOMERIMAGE' => $this->security->xss_clean($_POST['image']),                        
			   );
			   $this->session->set_userdata($newdata);

			   sendNotificationEditProfile(array('user_id'=>$this->security->xss_clean($_POST['user_id'])),'WEB');
...
function tofa(){
	$_POST['user_id'] = $this->session->userdata('CUSTOMERID');
    $this->home_model->update_tofa($this->security->xss_clean($_POST));
